...
ADMIN_ID = 8273386412  # ID разработчика (сюда приходят запросы)
...
class FSM(StatesGroup):
    registration = State()
    wait_approval = State()  # Состояние ожидания одобрения админом
...
async def process_registration(message: types.Message, state: FSMContext):
    ...
    # Запрос разработчику (кнопки несут в себе Telegram ID юзера для управления)
...
async def admin_approve(callback: types.CallbackQuery):
    ...
    try:
        # Меняем состояние пользователя на выбор режима
...
async def main():
    ...
    logging.info("Бот с админ-подтверждением успешно запущен!")
